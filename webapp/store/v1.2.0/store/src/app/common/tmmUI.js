...
.controller('SilderController', ['$scope', '$element', function($scope, $element) {
  ...
  function bindDOM() {
    ...
    var moveHandler = function(ev) {
      ...
      var m = i + 3;
      for (i; i < m; i++) {
        if (elms[i])(elms[i].style.webkitTransition = '-webkit-transform 0s ease-out');
        if (elms[i])(elms[i].style.webkitTransform = 'translate3d(' + ((i - index) * w + offsetX) + 'px, 0, 0)');
      }
  ...
  function goIndex(n) {
    ...
    if (elms[index - 1])(elms[index - 1].style.webkitTransition = '-webkit-transform 0.2s ease-out');
    if (elms[index + 1])(elms[index + 1].style.webkitTransition = '-webkit-transform 0.2s ease-out');

    elms[index].style.webkitTransform = 'translate3d(0, 0, 0)';
    if (elms[index - 1])(elms[index - 1].style.webkitTransform = 'translate3d(-' + w + 'px, 0, 0)');
    if (elms[index + 1])(elms[index + 1].style.webkitTransform = 'translate3d(' + w + 'px, 0, 0)');
