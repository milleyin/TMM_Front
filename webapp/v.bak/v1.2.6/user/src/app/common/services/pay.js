...
.factory('payFunc', function($ionicPopup, Resource, appFunc, device, ENV) {
  return {
    /**
     * 支付方式付款
     * @param  {[type]} id    订单id
    ...
    payOrder: function(id,type,  money, sfn, efn, ffn) {
      ...
      function payZhifubao() {
        ...
        Resource.orderPay(token, sfn, efn);
        closeModel();
      }
      ...
      function payWx() {
        ...
        return;
        // Resource.weixinPay(id).then(function(data){
        //   console.log(data);
        //   device.weixinPay(data.wxpay);
        // }, function(data) {
