...
//我发起的活动列表页
.controller('MyCreateActCtrl', function($scope, $http, $ionicHistory, $filter, $ionicLoading, data, Resource, autoRefresh, appFunc, payFunc, uiCalendar, modify) {
  $scope.model = {
    nextPage: '',
    list_data: []
  }
...
//我发起的觅趣支付
.controller('CreateActCtrl', function($scope, $http, $ionicHistory, $stateParams, $ionicPopup, $location, $state, $filter, data, Resource, appFunc, payFunc, security, uiCalendar, backStep, modify) {
  ...
  function orderPay(id, price) {
    ...
    payFunc.payOrder(id, 4, price, function(dataRes, status){
      if (dataRes.status == 1) {
