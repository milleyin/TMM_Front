...
.controller('DetailCtrl', function($scope, $sce, appFunc, $stateParams, data, bookInfo, farmList, device, ENV) {
  ...
  function share() {
    setTimeout(function() {
      device.share($scope.model.name, window.location.href, $scope.model.share_image, $scope.model.page_info);
    }, 500)
  };
...
// 点的农产品列表
.controller('FarmListCtrl', function($scope, $stateParams, Resource, farmList) {
  if (farmList.list) {
...
.directive('shopBanner', function($window, Resource, $ionicLoading, $ionicActionSheet, $ionicPopup, appFunc, ENV, modify) {
      ...
      scope.doCall = function() {
        ...
        var hideSheet = $ionicActionSheet.show({
          ...
          buttonClicked: function(index) {
            // if (index === 0) {
            //   //联系运营商
            //   appFunc.tmmWxCallPhone(model.manage_phone);
            // }
            // if (index === 1) {
            //   //联系田觅觅客服
            //   appFunc.tmmWxCallPhone(model.tmm_phone);
            // }
            return false;
          }
        });
