...
.controller('MyOrderCtrl', function($scope, $ionicScrollDelegate, $ionicLoading, orderType, data, Resource, appFunc, payFunc, device, ENV) {
  ...
  $scope.orderPay = function(id, type, price) {
    ...
    payFunc.payOrder(id, type, price, function(dataRes) {
      try {
        if (ENV.iOS) { //ios
...
//订单详情页
.controller('OrderDetailCtrl', function($scope, $stateParams, $ionicModal, appFunc, QRCode, Resource, data, payFunc, ENV) {
  $scope.actback = 0;
  ...
  $scope.orderPay = function(id, price) {
    payFunc.payOrder(id, $scope.model.status.order_type.value, price, function(dataRes) {
      try {
        if (ENV.iOS) { //ios
          connectWebViewJavascriptBridge(function(bridge) {
            bridge.callHandler('ObjcCallback', {
