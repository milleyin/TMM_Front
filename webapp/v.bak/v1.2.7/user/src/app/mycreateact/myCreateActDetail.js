...
//我发起的活动详情页
.controller('MyCreateActDetailCtrl', function($scope, $http, $ionicActionSheet, $filter, $sce, $stateParams, $ionicPopup, $ionicLoading, data, modify, Resource, appFunc, uiCalendar, security, device, ENV) {
  ...
  var shareUrl = ENV.apiEndpoint + '/index.php?r=site/share&share=' + $scope.model.shops.value;
  ...
  $scope.shareFriend = function() {
    if (ENV.device === 'weixin') {
      ...
      $ionicPopup.alert({
        title: '点击右上角分享邀请好友',
        okText: '取消',
        cssClass: 'tmm-ionic-alert sharebg'
      });

    } else {

      device.shareMsg($scope.model.name, desc, $scope.model.image, shareUrl);
  ...
  if (ENV.device === 'weixin') {
    ...
    function share() {
      setTimeout(function() {
        ...
        device.share($scope.model.name, shareUrl, $scope.model.image, desc);
