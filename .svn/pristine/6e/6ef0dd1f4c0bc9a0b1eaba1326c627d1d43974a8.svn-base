...
.config(['$stateProvider', function($stateProvider) {
  $stateProvider
    .state('tab.login', {
      url: '/login',
      templateUrl: 'app/my/login.html',
      controller: 'LoginCtrl',
    });

}])

.controller('LoginCtrl', function($scope, $http, $timeout, $location, API_CONFIG, Resource, appFunc) {
  //控制登录方式显示
  ...
  function accLoginView() {
    ...
    $scope.showAccVerifyCode = function() {
      Resource.showAccCode().then(function(dataRes) {
        $http.get(API_CONFIG.url + dataRes.verifyCode).success(function(dataRes) {
          $scope.accLoginData.g_hash = dataRes.hash1;
          $scope.accLoginData.verifyCodeUrl = API_CONFIG.url + dataRes.url;
        }, function(dataRes) {
    ...
    $scope.showMobileVerifyCode = function() {
      Resource.showMobCode().then(function(dataRes) {
        ...
        $http.get(API_CONFIG.url + dataRes.verifyCode).success(function(dataRes) {
          ...
          $scope.mobLoginData.verifyCodeUrl = API_CONFIG.url + dataRes.url;
        }, function(dataRes) {
