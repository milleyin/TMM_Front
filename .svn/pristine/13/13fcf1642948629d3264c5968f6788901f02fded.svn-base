...
var httpService = {
  ...
  /**
   * @method getAdIndex
   * @description type=0获取首页广告 type=1获取当季热销
   *
   * @param    {Int} type 类型
   * @param    {Functoin} successCb 成功后的回调函数
   * 参数为：dataRes 响应回来的数据 statusCode 状态码
   ...
   * @author Moore Mo
   */
  getAdIndex: function(type, successCb, errorCb) {
    var type = type || 0;
    $$.ajax({
      method: 'GET',
      url: apiServerHost + '/index.php?r=api/ad/index&type=' + type,
      xhrFields: {
        withCredentials: true
      },
      success: function(dataRes, statusCode) {
        ...
        log.info('getAdIndex---', dataRes);

        successCb(dataRes, statusCode);
      },
      error: function(dataRes, statusCode) {
        log.error('getAdIndex---', dataRes);
   ...
   * @author Moore Mo
   */
  getHotAct: function(successCb, errorCb) {
    $$.ajax({
      method: 'GET',
      url: apiServerHost + '/index.php?r=api/shops/hot',
      xhrFields: {
        withCredentials: true
      },
      success: function(dataRes, statusCode) {
        ...
        log.info('getHotAct---', dataRes);

        successCb(dataRes, statusCode);
      },
      error: function(dataRes, statusCode) {
        log.error('getHotAct---', dataRes);
  ...
  /**
   * @method getNearby
   * @description 获取首页推送的周边游
   * 
   * @param    {String} link 请求的链接
   * @param    {Functoin} successCb 成功后的回调函数
   * 参数为：dataRes 响应回来的数据 statusCode 状态码
   *
   * @param    {Functoin} errorCb   失败后的回调函数
   ...
   * @author Moore Mo
   */
  getNearby: function(link, successCb, errorCb) {
    var link = link || apiServerHost + '/index.php?r=api/shops/nearby';
    $$.ajax({
      method: 'GET',
      url: link,
      xhrFields: {
      ...
      success: function(dataRes, statusCode) {
        ...
        log.info('getNearby---', dataRes);

        successCb(dataRes, statusCode);
      },
      error: function(dataRes, statusCode) {
        log.error('getNearby---', dataRes);
