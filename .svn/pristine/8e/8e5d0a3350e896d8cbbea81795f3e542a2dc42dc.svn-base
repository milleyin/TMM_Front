...
var Dot = {
  ...
  getDotSelect: function() {
    ...
    // 请求获取点的详情
    httpService.getDotList(
      selectUrl + '&select_dot_thrand=dot&page=1',
      function(dataRes, statusCode) {
        tmmApp.hideIndicator();
        if (dataRes.status == 1) {
  ...
  createAct: function() {
    ...
    // 创建旅游活动（多个点）的数据格式，要发给后端保存起来，方便编辑活动时使用
