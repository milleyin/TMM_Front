var httpService = require('../services/httpService'),
    ...
    actMoreModule = require('../actMore/actMore'),
    hotMoreModule = require('../hotMore/hotMore');
...
var recommend = {
    ...
    indexView: function() {
        ...
        function getAdIndex() {
            httpService.getAdIndex(0,
              function(dataRes, status) {
                    tmmApp.hideIndicator();
                    recommendView.indexAdsView(dataRes.data);
                },
                function(dataRes, status) {
        ...
        // 获取首页当季热销
        function getHot() {
            httpService.getAdIndex(1,
              function(dataRes, status) {
                recommendView.indexHotView(dataRes.data);
                },
                function(dataRes, status) {
                    tmmApp.alert('网络超时，请重试', '');
                });
        ...
        // 获取首页觅趣活动
        function getHotAct() {
            httpService.getHotAct(
                function(dataRes, status) {
                    recommendView.indexHotActView(dataRes.data);
                },
                function(dataRes, status) {
                    tmmApp.alert('网络超时，请重试', '');
                });
        ...
        // 获取首页周边景区
        function getNearby() {
            httpService.getNearby('',
                function(dataRes, status) {
                    recommendView.indexNearbyView(dataRes.data);
                },
                function(dataRes, status) {
                    tmmApp.alert('网络超时，请重试', '');
                });
    ...
    bindEvent: function() {
        ...
        var bindings = [{
            ...
            element: '#recommendView',
            selector: '.tmm-act-special-more',
            event: 'click',
            handler: actMoreModule.init
        }, {
            element: '#recommendView',
            selector: '.tmm-hot-special-more',
            event: 'click',
            handler: hotMoreModule.init
        }];
