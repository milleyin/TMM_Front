...
// 声明配置
var config = {
  host: gulp.env.host || '172.16.1.71',
  port: gulp.env.port || 3000,
};
...
gulp.task('webpack:build', function(callback) {
  ...
  prodCompiler.run(function(err, stats) {
    ...
    //fse.copySync('src/tpls', 'www/tpls');
    //fse.copySync('src/pages', 'www/pages');
  ...
  devCompiler.run(function(err, stats) {
    ...
    fse.readFile(path.join('www', 'index.html'), 'utf-8', function(err, html) {
      ...
      fse.readJson('stats.json', function(err, stats) {
        ...
        var content = html.replace('localhost:3000', config.host + ':' + config.port);
...
gulp.task('webpack-dev-server', function(callback) {
  ...
  new WebpackDevServer(webpack(serverConfig), {
    ...
    gutil.log('[webpack-dev-server]', 'http://' + config.host +':' + config.port + '/webpack-dev-server/index.html');
  });
});
