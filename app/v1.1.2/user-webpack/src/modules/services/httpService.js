...
var httpService = {
   ...
   */
  getFareDot: function(id, successCb, errorCb) {
    $$.ajax({
      method: 'GET',
      url: apiServerHost + '/index.php?r=api/order/fare_dot&id=' + id,
      xhrFields: {
        withCredentials: true
      },
      success: function(dataRes, statusCode) {
  ...
  /**
   * 获取线的订单信息（创建订单模块）
   * @param  {[type]} id        线订单的id
   * @param  {[type]} successCb [description]
   * @param  {[type]} errorCb   [description]
   * @return {[type]}           [description]
   */
  getLineOrder: function(id, successCb, errorCb) {
    var url = apiServerHost + '/index.php?r=api/order/fare_thrand&id=' + id;
    $$.ajax({
      method: 'GET',
      url: url,
      xhrFields: {
   ...
   *
   * @author Moore Mo
   * @datetime 2015-11-02T14:13:24+0800
   */
  getDotList: function(successCb, errorCb) {
    $$.ajax({
      method: 'GET',
      url: apiServerHost + '/index.php?r=api/shops/index&select_dot_thrand=dot',
      xhrFields: {
        withCredentials: true
      },
      success: function(dataRes, statusCode) {
  ...
  /**
   * @method getDotListToPage
   * @description 获取点的列表(分页)
   *
   * @param    {[type]}                 url       [description]
   * @param    {[type]}                 successCb [description]
   * @param    {[type]}                 errorCb   [description]
   *
   * @author Moore Mo
   * @datetime 2015-11-02T14:14:00+0800
   */
  getDotListToPage: function(url, successCb, errorCb) {
    ...
    $$.ajax({
      ...
      success: function(dataRes, statusCode) {
        ...
        log.info('getDotListToPage---', dataRes);

        successCb(dataRes, statusCode);
      },
      error: function(dataRes, statusCode) {
        log.error('getDotListToPage---', dataRes);
