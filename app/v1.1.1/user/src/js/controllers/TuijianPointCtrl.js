tmmApp.controller('TuijianPointCtrl',['$scope','$rootScope','$http','$location',function($scope, $rootScope, $http,$location) {
    ...
    // 获取点的信息
    $http.get($rootScope.tuiJianPointUrl).success(function(data) {
        if (data.status != 1) {
            $location.path('tuijian');
            return;
        }
